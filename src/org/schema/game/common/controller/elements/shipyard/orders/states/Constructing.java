...
public class Constructing extends ShipyardState {
  ...
  public boolean onEnterS() {
    ...
    if (!isLoadedFromTag()) {
      ...
      //			System.err.println("LOADED MAP "+getEntityState().currentMapTo.getTotalAmount()+";
      // "+getEntityState().currentMapTo);
    }
    ships.clear();
    newShip = null;
  ...
  public boolean onUpdate() throws FSMException {
    ...
    if (!checkedName) {
      ...
      boolean existsEntity =
          ((GameServerState) getEntityState().getState())
              .existsEntity(
                  EntityType.SHIP,
                  getEntityState().currentName
                      + (getEntityState().spawnDesignWithoutBlocks ? "_SY_TEST" : ""));

      LogUtil.sy()
          .fine(
              getEntityState().getSegmentController()
                  + " "
                  + getEntityState()
                  + " "
      ...
      if (existsEntity) {
        getEntityState()
            .sendShipyardErrorToClient(
                Lng.str("Construction interrupted!\nOriginal Ship name already exists!"));
    ...
    //			boolean checkSectorCollisionWithChildShape =
    // currentDocked.getCollisionChecker().checkSectorCollisionWithChildShape();
    //
    //			if(checkSectorCollisionWithChildShape){
    //				getEntityState().sendShipyardErrorToClient(Lng.str("Cannot construct! Design is
    // overlapping with another structure!"));
    //				stateTransition(Transition.SY_ERROR);
    //				return false;
    //			}
    //
    ...
    if (spawnedShip > 0) {
      ...
      if (((GameServerState) getEntityState().getState())
              .getLocalAndRemoteObjectContainer()
              .getLocalObjects()
              .containsKey(lastSpawnedId)
        ...
        SegmentPiece pointUnsave =
            spawned.getSegmentBuffer().getPointUnsave(Ship.core); // autorequest true previously

        System.err.println(
            "[SERVER][SHIPYARD][CONSTRUCTING] construction done. Core: " + pointUnsave);
        LogUtil.sy()
            .fine(
                getEntityState().getSegmentController()
                    + " "
                    ...
                    + " "
                    + this.getClass().getSimpleName()
                    + ": construction: waiting for construction DONE: CORE: "
                    + pointUnsave);
            ...
            LogUtil.sy()
                .fine(
                    getEntityState().getSegmentController()
                        ...
                        + " "
                        + this.getClass().getSimpleName()
                        + ": construction: waiting for construction Force request of 0 0 0: "
                        + pointUnsave);
            ...
            LogUtil.sy()
                .fine(
                    getEntityState().getSegmentController()
                        ...
                        + " "
                        + this.getClass().getSimpleName()
                        + ": construction: waiting for construction WAITING FOR SPAWN: "
                        + pointUnsave);
          }
        }
      } else {
        LogUtil.sy()
            .fine(
                getEntityState().getSegmentController()
                    ...
                    + " "
                    + this.getClass().getSimpleName()
                    + ": construction: waiting for construction to be done");
        System.err.println(
            "[SERVER][SHIPYARD][CONSTRUCTING] waiting for constructed ship to be loaded");
      }
    } else if (newShip != null && getEntityState().getCurrentDocked() == null) {

      boolean b =
          getEntityState().getShipyardCollectionManager().createDockingRelation(newShip, false);
      ...
      LogUtil.sy()
          .fine(
              getEntityState().getSegmentController()
                  ...
                  + " "
                  + this.getClass().getSimpleName()
                  + ": construction: ship spawned "
                  + newShip
                  + ": creating docking relation: "
                  + b);
      ...
      if (b) {
        ...
        for (Ship n : ships) {
          ...
          ((GameServerState) getEntityState().getState())
              .getController()
              .getSynchController()
              .addNewSynchronizedObjectQueued(n);
        }
        ...
        LogUtil.sy()
            .fine(
                getEntityState().getSegmentController()
                    ...
                    + " "
                    + this.getClass().getSimpleName()
                    + ": construction: ship successfully spawned and docked");
      ...
      currentDesign = null;
        ...
        && currentDesign == getEntityState().getCurrentDesign()
        && currentDocked != null
        && currentDocked.isVirtualBlueprint()
        && currentDocked == getEntityState().getCurrentDocked()) {
      ...
      LogUtil.sy()
          .fine(
              getEntityState().getSegmentController()
                  ...
                  + " "
                  + this.getClass().getSimpleName()
                  + ": construction: spawning new ship");

      if (getEntityState().spawnDesignWithoutBlocks
          || getEntityState().currentMapFrom.equals(getEntityState().currentMapTo)
          || getEntityState().getShipyardCollectionManager().isInstantBuild()) {
        ...
        LogUtil.sy()
            .fine(
                getEntityState().getSegmentController()
                    ...
                    + " "
                    + this.getClass().getSimpleName()
                    + ": construction: spawning new ship: blocks all available");
        ...
        if (currentDocked.isFullyLoadedWithDock()) {
          ...
          if (EntityRequest.existsIdentifierWOExc(getEntityState().getState(), fileName)) {
            getEntityState()
                .sendShipyardErrorToClient(Lng.str("A ship with this name already exists!"));
            stateTransition(Transition.SY_ERROR);
            assert (false);
          } else {
            ...
            SegmentPiece pointUnsave =
                getEntityState()
                    .getCurrentDocked()
                    .getSegmentBuffer()
                    .getPointUnsave(Ship.core); // autorequest true previously
            if (pointUnsave != null) {
              System.err.println(
                  "[SERVER][SHIPYARD][CONSTRUCING] core of design Design : " + pointUnsave);
              ...
              newShip =
                  ((Ship) currentDocked)
                      .copy(
                          ...
                          tr,
                          Lng.str("SHIPYARD_")
                              + getEntityState().getSegmentController().getUniqueIdentifier(),
                          factionId,
                          ships,
                          new EntityCopyData());
              ...
              */
              ShipManagerContainer oldManagerContainer =
                  ((Ship) currentDocked).getManagerContainer();
              ShipManagerContainer newManagerContainer = newShip.getManagerContainer();
              for (Map.Entry<Short, ModManagerContainerModule> entry :
                  oldManagerContainer.getModModuleMap().entrySet()) {
        ...
        if (currentDocked.isFullyLoadedWithDock()) {
          LogUtil.sy()
              .fine(
                  getEntityState().getSegmentController()
                      ...
                      + " "
                      + this.getClass().getSimpleName()
                      + ": construction: spawning new ship: aquiring blocks start step: "
                      + currentFill
                      + " / "
                      + currentDocked.getTotalElements()
                      + " blocks");
          ...
          while (tickCounter > ShipyardElementManager.CONSTRUCTION_TICK_IN_SECONDS) {
            ...
            //					System.err.println("TICK_COUNTER "+tickCounter+" /
            // "+ShipyardElementManager.CONSTRUCTION_TICK_IN_SECONDS);
            int toTake = Math.max(1, ShipyardElementManager.CONSTRUCTION_BLOCKS_TAKEN_PER_TICK);

            if (getEntityState().lastDeconstructedBlockCount > toTake
                && getEntityState().isInRepair) {
            ...
            for (short type : ElementKeyMap.keySet) {
              if (getEntityState().currentMapTo.get(type)
                  < getEntityState().currentMapFrom.get(type)) {

                short invType = type;
                if (ElementKeyMap.isValidType(type)
                    && ElementKeyMap.getInfoFast(type).getSourceReference() != 0) {
                  invType = (short) ElementKeyMap.getInfoFast(type).getSourceReference();
                }

                int goalOrig =
                    getEntityState().currentMapFrom.get(type)
                        - getEntityState().currentMapTo.get(type);
                int goal = goalOrig;

                int count =
                    ShipyardCollectionManager.DEBUG_MODE
                        ? goal
                        : inventory.getOverallQuantity(invType);
          ...
          getEntityState()
              .setCompletionOrderPercentAndSendIfChanged(
                  (double) currentFill / (double) currentDocked.getTotalElements());
          ...
          LogUtil.sy()
              .fine(
                  getEntityState().getSegmentController()
                      ...
                      + " "
                      + this.getClass().getSimpleName()
                      + ": construction: spawning new ship: aquiring blocks end step: "
                      + currentFill
                      + " / "
                      + currentDocked.getTotalElements()
                      + " blocks");
          ...
          LogUtil.sy()
              .fine(
                  getEntityState().getSegmentController()
                      ...
                      + " "
                      + this.getClass().getSimpleName()
                      + ": construction: spawning new ship: aquiring blocks ERROR: Design not fully"
                      + " loaded");
        }
      } else {
        LogUtil.sy()
            .fine(
                getEntityState().getSegmentController()
                    ...
                    + " "
                    + this.getClass().getSimpleName()
                    + ": new ship exists, but yard is idle: "
                    + newShip);
      }
    } else if (newShip == null) {
      LogUtil.sy()
          .fine(
              getEntityState().getSegmentController()
                  ...
                  + " "
                  + this.getClass().getSimpleName()
                  + ": construction interrupted. design no longer docked!");
      getEntityState()
          .sendShipyardErrorToClient(
  ...
  public void pullResources() {
    ...
    long currentStep =
        yard.getSegmentController().getState().getController().getServerRunningTime()
            / ManagerContainer.TIME_STEP_STASH_PULL;
      ...
      for (short type : ElementKeyMap.keySet) {
        if (getEntityState().currentMapTo.get(type) < getEntityState().currentMapFrom.get(type)) {
          ...
          if (ElementKeyMap.isValidType(type)
            ...
            invType = (short) ElementKeyMap.getInfoFast(type).getSourceReference();
            additionalGoal =
                getEntityState().currentMapFrom.get(invType)
                    - getEntityState().currentMapTo.get(invType);
          }

          int goal =
              getEntityState().currentMapFrom.get(type) - getEntityState().currentMapTo.get(type);
          goal += additionalGoal;
          ...
          if (have < goal) {
            ...
            for (short s : ElementKeyMap.inventoryTypes) {
              LongOpenHashSet connectedInventoriesToThis =
                  yard.getSegmentController()
                      .getControlElementMap()
                      .getControllingMap()
                      .get(yard.getControllerElement().getAbsoluteIndex())
                      .get(s);
              if (connectedInventoriesToThis != null) {
                for (long l : connectedInventoriesToThis) {
                  ...
                  if (toTake > 0 && have < goal) {
                    Inventory pullFromInv =
                        yard.getContainer().getInventory(ElementCollection.getPosIndexFrom4(l));
                    ...
                    if (pullFromInv != null) {
                      ...
                      if (take > 0 && (take = inventory.canPutInHowMuch(invType, take, -1)) > 0) {
                        ...
                        changeMap
                            .getInv(inventory)
                            .add(inventory.putNextFreeSlotWithoutException(invType, take, -1));
                        have += take;
                      }
                    }
                  } else {
  ...
  public void onShipyardRemoved(Vector3i shipyardControllerPos) {
    ...
    getEntityState()
        .currentMapTo
        .spawnInSpace(getEntityState().getSegmentController(), shipyardControllerPos);
  }

  @Override
  public String getClientShortDescription() {
    return Lng.str(
        "Constructing (%d blocks every %d sec)",
        ShipyardElementManager.CONSTRUCTION_BLOCKS_TAKEN_PER_TICK,
        (long) ShipyardElementManager.CONSTRUCTION_TICK_IN_SECONDS);
  }
}
